      ...
      {showPlayersHeader && (
        <>
          ...
          {visiblePlayers.map((player, idx) => {
            ...
            const displayPlusMinus = ((player.plusMinus >= 0 ? "+" : "") + String(player.plusMinus)).padStart(4);
            const displayShots = String(player.shots).padStart(4);
            const displayShootingPct = (player.shootingPctg > 0 ? player.shootingPctg.toFixed(1) + "%" : "0.0%").padStart(5);
      ...
      {showGoaliesHeader && (
        <>
          ...
          {visibleGoalies.map((goalie, idx) => {
            ...
            const displayName = `${goalie.firstName.charAt(0)}. ${goalie.lastName}`.slice(0, 20).padEnd(20);
            ...
            const displaySavePct = (goalie.savePct > 0 ? goalie.savePct.toFixed(3) : "0.000").padStart(5);
