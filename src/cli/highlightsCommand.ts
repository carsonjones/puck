...
export async function runHighlightsCommand(options: HighlightsCommandOptions) {
  ...
  if (options.format === 'table') {
    ...
    for (const h of trimmed) {
      console.log(
        `${h.away} @ ${h.home} | ${h.playerName} (${h.teamAbbrev}) ${h.strength.toUpperCase()} ${h.timeInPeriod}`,
      );
...
async function getBrightcoveSources(
  clipId: string,
): Promise<{ mp4Url: string | null; hlsUrl: string | null }> {
  const configRes = await fetch(
    'https://players.brightcove.net/6415718365001/default_default/config.json',
  );
