import { BaseURLWeb, SortOrder } from "./constants.js";
import { getCurrentSeasonId } from "./formatters.js";
import type {
  BoxscoreResponse,
...
export class NhlClient {
  ...
  async getTeamByIdentifier(identifier: string): Promise<TeamInfo> {
    ...
    const byName = teams.teams.find(
      (team) => team.abbreviation.toLowerCase() === lowered || team.name.default.toLowerCase() === lowered
    );
